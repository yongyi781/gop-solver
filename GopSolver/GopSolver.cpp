...
std::array<std::vector<Point>, 6> altarSpawns{ airSpawns, newMindSpawns, waterSpawns, newEarthSpawns, fireSpawns, bodySpawns };
...
void doAverageSolvingCosts(int altar, Point playerLocation, std::function<bool(Point)> filter)
{
	...
	for (auto spawn : filteredSpawns)
	{
		...
		std::cout << "Spawn " << ++spawnNum << " of " << filteredSpawns.size() << ": " << spawn.toString() << ", " << cost << std::endl;
		costSum += cost;
	}

	cout << "Average cost from " << playerLocation.toString() << ": " << (double)costSum / filteredSpawns.size() << endl;
}

std::string parFileToGrid(std::string fileName)
{
	GopArray<int> grid{};
...
int _tmain()
{
	//doAverageSolvingCosts(2, { 0, -2 }, [](Point p) { return true; });
	std::string fileName = R"(C:\Users\Yongyi\OneDrive\Documents\RS\GOP\Solver\water (0,-2).txt)";
	std::string result = parFileToGrid(fileName);
