...
int spawnBadness(int seed, int numSpawns = 50)
{
	...
	for (int i = 0; i < numSpawns; ++i)
	{
		...
		int distance = GopEngine::distanceToAltar(spawn);
		if (spawn == Point(-3, -7))
			total += 3;
...
void findGoodSpawns()
{
	GopEngine::loadAltarFromFile("..\\GopSolverLib\\air.txt");
...
void doReachable()
{
	GopEngine::loadAltarFromFile("..\\GopSolverLib\\water.txt");
	const int N = 2;
	typedef std::tuple<Point, Point, int> Data;
	std::vector<Data> data;
	for (int y = -N; y <= N; ++y) {
		for (int x = -N; x <= N; ++x) {
			if (GopEngine::get(x, y) != Tile::Floor)
				continue;
			for (int y2 = -N; y2 <= N; ++y2) {
				for (int x2 = -N; x2 <= N; ++x2) {
					if (GopEngine::get(x2, y2) != Tile::Floor)
						continue;
					// Count spawns reachable from spot
					int total = 0;
					std::for_each(std::begin(waterSpawns), std::end(waterSpawns), [&](Point p) {
						if (GopEngine::canReach(Point(x, y), p) || GopEngine::canReach(Point(x2, y2), p))
							++total;
					});
